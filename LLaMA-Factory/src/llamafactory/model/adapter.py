...
def _setup_full_tuning(
    ...
    for name, param in model.named_parameters():
        if not any(forbidden_module in name for forbidden_module in forbidden_modules):
            # for V100, and modern models with bf16, we need to cast the params to float16
            cast_trainable_params_to_float16 = True
            #print(f"cast_trainable_params_to_fp32:{cast_trainable_params_to_fp32}")
            if cast_trainable_params_to_fp32:
                logger.info("Here we cast param to float32")
                param.data = param.data.to(torch.float32)
            elif cast_trainable_params_to_float16:
                param.data = param.data.to(torch.float16)
        else:
            param.requires_grad_(False)
        #logger.info(f"Jay Test: {name} and {param}")
...
def init_adapter(
    ...
    """
    print(f"init_adapter is called !!!")
